...
static_assert(ATOMIC_LLONG_LOCK_FREE, "x86_64 ILP32 has lock-free llong");
typedef unsigned long long stats_uint_t;
typedef atomic_ullong stats_t;
#define PRISTATS "llu"
#else
static_assert(ATOMIC_POINTER_LOCK_FREE, "has lock-free pointer");
...
// stats_own_inc: convenience add(1)
#define stats_own_inc(_x) stats_own_add((_x), 1U)
