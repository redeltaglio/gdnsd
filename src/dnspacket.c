...
static dnsp_ctx_t* dnspacket_ctx_init(dnspacket_stats_t** stats_out, const bool is_udp, const bool udp_is_ipv6, const bool tcp_pad, const unsigned tcp_timeout_secs)
{
    ...
    if (gcfg->dnssec_enabled)
        ctx->nxdc = nxdc_new(gcfg->dnssec_nxd_cache_scale, gcfg->dnssec_nxd_sign_rate, gcfg->dnssec_max_active_zsks);
    ctx->is_udp = is_udp;
...
F_NONNULL
static unsigned encode_rrs_signed_deleg(dnsp_ctx_t* ctx, const ltree_node_t* node, unsigned offset)
{
    gdnsd_assert(offset);
    gdnsd_assert(ctx->txn.edns.do_bit);

    // These rrsets were pre-ordered during ltree post-processing:
    ltree_rrset_raw_t* ns = &node->c.rrsets->raw;
    gdnsd_assert(ns);
    gdnsd_assert(ns->gen.type == DNS_TYPE_NS);
    ltree_rrset_raw_t* dnssec = &ns->gen.next->raw;
    gdnsd_assert(dnssec);
    ...
    gdnsd_assert(dnssec->num_comp_offsets);
    gdnsd_assert(dnssec->comp_offsets);
...
static unsigned encode_rrs_raw(dnsp_ctx_t* ctx, const unsigned offset, const ltree_rrset_raw_t* rrset)
{
    ...
    if (!do_bit && rrset->num_rrsig) {
        ...
        if (rrset->num_addtl) {
            ...
            gdnsd_assert(data_len > glue_offset);
...
F_NONNULL
static const ltree_rrset_t* find_rrset(const ltree_rrset_t* rrset, unsigned rrtype)
{
    do {
...
F_NONNULL
static unsigned do_auth_response(dnsp_ctx_t* ctx, const search_result_t* res, unsigned offset)
{
    uint8_t* packet = ctx->txn.pkt->raw;
    gdnsd_assert(packet);
    ...
    const ltree_node_zroot_t* auth = res->auth;
    gdnsd_assert(auth);
    const ltree_node_t* dom = NULL;
    const ltree_node_t* enxd = NULL;
    const ltree_rrset_t* rrsets = NULL;
    if (res->dom) {
        if (res->dom->c.explicit_nxd) {
            enxd = res->dom;
            gdnsd_assert(enxd->c.rrsets);
            gdnsd_assert(enxd->c.rrsets->gen.count);
            gdnsd_assert(!enxd->c.rrsets->gen.type);
            gdnsd_assert(!enxd->c.rrsets->gen.next);
        } else {
            dom = res->dom;
            rrsets = dom->c.rrsets;
        }
    ...
    if (rrsets) {
        ...
        if (ctx->txn.qtype != DNS_TYPE_ANY) {
            const ltree_rrset_t* rrset = find_rrset(rrsets, ctx->txn.qtype);
            if (rrset) {
                if (rrset->gen.count)
    ...
    if (!ctx->txn.ancount) { // NODATA or NXDOMAIN handling:
        ...
        if (ctx->txn.edns.do_bit && auth->sec) {
            if (nxd_case) {
                ...
                unsigned resp_len = 0;
                if (enxd) {
                    ltree_rrset_enxd_t* enxd_rrset = &enxd->c.rrsets->enxd;
                    resp_len = enxd_rrset->data_len;
                    memcpy(&packet[offset], enxd_rrset->data, enxd_rrset->data_len);
                } else {
                    gdnsd_assert(ctx->txn.lqname[0] > res->comp_fixup_dnssec_nxd);
                    ...
                    bool hit = false;
                    gdnsd_assert(ctx->nxdc && gcfg->dnssec_enabled);
                    resp_len = nxdc_synth(ctx->nxdc, auth->sec,  nxd_name, &packet[offset], &hit, res->gen, nxd_name_len);
                    if (resp_len) {
                        if (hit) {
                            stats_own_inc(&ctx->stats->dnssec_nxd_hit);
                        } else {
                            stats_own_inc(&ctx->stats->dnssec_nxd_synth);
                            stats_own_add(&ctx->stats->dnssec_nxd_signs, num_zsks);
                        }
                    } else {
                        stats_own_inc(&ctx->stats->dnssec_nxd_drop);
                ...
                const ltree_rrset_t* nsec = find_rrset(rrsets, DNS_TYPE_NSEC);
                gdnsd_assert(nsec); // all NODATA cases for signed zones have NSEC
