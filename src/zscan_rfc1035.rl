...
F_NONNULL
static void set_uval8(zscan_t* z)
{
    set_uval(z);
    if (z->uval > 255)
...
F_NONNULL
static void set_uval16(zscan_t* z)
{
    set_uval(z);
    if (z->uval > 65535)
...
F_NONNULL
static bool zscan_do(ltree_node_zroot_t* zroot, const uint8_t* origin, const char* fn, const unsigned def_ttl_arg)
{
...
static unsigned clamp_ttl(zscan_t* z, const uint8_t* dname, const unsigned rrtype, const unsigned ttl)
{
    if (ttl > gcfg->max_ttl) {
...
F_NONNULL
static void rec_1name(zscan_t* z, unsigned rrtype)
{
    z->ttl = clamp_ttl(z, z->lhs_dname, rrtype, z->ttl);
...
static void rec_ds(zscan_t* z)
{
    ...
    gdnsd_put_una16(ntohs(rdlen), z->rfc3597_data);
    gdnsd_put_una16(ntohs(key_tag), &z->rfc3597_data[2]);
...
F_NONNULL
static void dsdig_setup(zscan_t* z)
{
...
F_NONNULL
static void dsdig_octet(zscan_t* z)
{
    if (z->rfc3597_data_written == z->rfc3597_data_len)
...
F_NONNULL
static void zone_add_zsk(zscan_t* z, unsigned algid)
{
...
bool zscan_rfc1035(ltree_node_zroot_t* zroot, const char* fn)
{
