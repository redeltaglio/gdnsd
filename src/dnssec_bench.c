...
static void bench_run(uint8_t* out, const zsk_t* zsk, const uint8_t* in, const unsigned inlen, const unsigned long iters)
{
...
int main(int argc, char* argv[])
{
    ...
    while ((optchar = getopt(argc, argv, "i:s:")) != -1) {
        ...
        case 'i':
            ...
            if (!iters || iters > 100000000LU)
                usage(argv[0]);
            break;
        case 's':
            dsize = strtoul(optarg, NULL, 10);
            if (!dsize || dsize > 16384LU)
                usage(argv[0]);
            break;
        default:
            usage(argv[0]);
    ...
    zsk_t* zsks = sodium_malloc(16U * sizeof(*zsks));
    unsigned num_zsks = 0;
    uint8_t* dnskey_rdata = dnssec_alg_init_zsk(&zsks[num_zsks++], DNSSEC_ALG_ED25519, 0);
    free(dnskey_rdata);
#if defined HAVE_GNUTLS || defined HAVE_LIBCRYPTO
