...
#else
        log_fatal("Config option 'dnssec_deterministic_ecdsa' requires GnuTLS support at build time");
#endif
    dnssec_alg_init_global();
}
...
bool dnssec_add_ephemeral_zsk(ltree_node_zroot_t* zroot, const unsigned algid)
{
    dnssec_t* sec = zroot->sec;
    if (!sec) {
    ...
    zsk_t* zsk = &sec->zsks[sec->num_zsks];
...
static unsigned dnssec_rrsigs_make_preimage(uint8_t* out, const dnssec_t* sec, unsigned lcount, unsigned rrtype, uint32_t ttl)
{
    gdnsd_assert(lcount < 128U);
    gdnsd_assert(rrtype < 65536U);
    unsigned offs = 0;
...
static unsigned dnssec_rrsigs_len(const dnssec_t* sec, const unsigned preimage_len, const unsigned rrsig_comp_offset)
{
    gdnsd_assert(sec->zsks && sec->num_zsks);
...
static unsigned dnssec_rrsigs_make(uint8_t* out, uint8_t* sign_buf, const dnssec_t* sec, unsigned sign_buf_len, unsigned preimage_len, unsigned rrsig_comp_offset, uint32_t ttl)
{
    gdnsd_assert(rrsig_comp_offset < 0x4000);
...
uint8_t* dnssec_sign_rrset(const ltree_node_t* node, ltree_rrset_raw_t* raw, const dnssec_t* sec)
{
    ...
    const unsigned written = dnssec_rrsigs_make(d, sign_buf, sec, sbuf_len, rrsig_pre_len, rrsig_comp_offset, raw->ttl);
...
{
    gdnsd_assert(owner_name_len && owner_name_len < 256U);
    gdnsd_assert(zone_name_len && zone_name_len < 256U);
    ...
    while (owner_name_len > zone_name_len) {
        ...
        gdnsd_assert(owner_name_len > adjust);
        owner_name += adjust;
...
void dnssec_nxd_fixup(const dnssec_t* sec, uint8_t* buf, const unsigned offset, const unsigned fixup)
{
    ...
    const unsigned rrsig_comp_offset = fixup ? offset : 12U;
    for (unsigned i = 0; i < sec->num_zsks; i++) {
...
{
    gdnsd_assert(nxd_name_len >= 3U); // can't be root-of-dns, and all other cases are 3+ bytes, e.g. "\1a\0"
    gdnsd_assert(nxd_name[0] != 0U); // can't be root-of-dns
    ...
    gdnsd_assert(offset > nsec_wire_start);
...
void dnssec_node_add_nsec(ltree_node_t* node, const dnssec_t* sec)
{
    ...
    while (*store_at) {
        ...
        if (!node->c.zone_cut_deleg || rrtype == DNS_TYPE_NS) {
            if (rrtype >= 512U && !all_wins) {
                gdnsd_assert(wins == two_wins);
                all_wins = wins = xcalloc_n(34U, 256U);
                gdnsd_assert(sizeof(two_wins) == 68U);
    ...
    ltree_rrset_raw_t* raw = xcalloc(sizeof(*raw));
    *store_at = (ltree_rrset_t*)raw;
