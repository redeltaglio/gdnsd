...
F_WUNUSED F_NONNULL
static bool ltree_add_rr_raw(ltree_node_t* zroot, const uint8_t* dname, uint8_t* data, const unsigned data_size, const char* rrtype_desc, unsigned ttl, unsigned rrtype)
{
    ...
    while (*store_at) {
        if ((*store_at)->gen.type == rrtype) {
            ...
            gdnsd_assert(rrset->data); // we asserted count earlier
...
bool ltree_add_rec_a(ltree_node_t* zroot, const uint8_t* dname, const uint32_t addr, unsigned ttl)
{
...
bool ltree_add_rec_soa_args(ltree_node_t* zroot, const uint8_t* dname, lt_soa_args args)
{
    ...
    const unsigned this_rr_rdlen = args.master[0] + args.email[0] + 20U;
    unsigned offs = 0;
    ...
    memcpy(&buf[offs], &args.master[1], args.master[0]);
    offs += args.master[0];
    memcpy(&buf[offs], &args.email[1], args.email[0]);
    offs += args.email[0];
