...
// comp_state_t tracks compressor state for an entire packet, through the
// storage of multiple names that might need compression, and which might be
// compression targets themselves
typedef struct {
    unsigned count;
    comp_target_t targets[COMPTARGETS_MAX];
} comp_state_t;
...
// comp_state_t endures, as comp_state_t will hold pointers into it.
F_NONNULL F_RETNN
static comp_state_t* comp_new(const uint8_t* qname, unsigned qname_len)
{
    comp_state_t* cs = xcalloc(sizeof(*cs));
    ...
    while (qname_len > 2) {
        gdnsd_assert(cs->count < COMPTARGETS_MAX);
...
F_NONNULL
static void comp_destroy(comp_state_t* cs)
{
    free(cs);
}
...
F_NONNULL
static unsigned comp_store_name(comp_state_t* cs, const uint8_t* name, uint8_t* store_at, const unsigned name_len, unsigned offset, bool make_targets)
{
    ...
    for (unsigned i = 0; i < cs->count; i++) {
        ...
        while (cs->targets[i].len < cur_name_len) {
            if (make_targets && cs->count < COMPTARGETS_MAX) {
                ...
                memmove(cs->targets + i + 1U, cs->targets + i, to_move * sizeof(comp_target_t));
...
F_NONNULL
static void raw_add_fixup(ltree_rrset_raw_t* rrset, const unsigned offset)
{
...
void comp_do_mx_cname_ptr(ltree_rrset_raw_t* rrset, const uint8_t* node_dname)
{
    ...
    gdnsd_assert(dname_status(node_dname) == DNAME_VALID);
    ...
    comp_state_t* cs = comp_new(node_name, node_name_len);
    ...
    for (unsigned i = 0; i < rrset->gen.count; i++) {
        ...
        gdnsd_assert(stored_len <= name_len);
        ...
        if (savings) {
            ...
            gdnsd_assert(rdlen == name_len + rdata_extra);
            rdlen -= savings;
...
static void fixup_all_lhs(ltree_rrset_raw_t* rrset, uint8_t* rrset_data, const unsigned num_rrs, const unsigned pkt_voffset)
{
...
void comp_do_soa(ltree_rrset_raw_t* rrset, const uint8_t* node_dname)
{
    ...
    // Copy and possibly-compress a pair of dnames for email/master
...
F_NONNULL
static ltree_dname_status_t ltree_search_name_zone(const uint8_t* name, unsigned name_len, ltree_node_zroot_t* zroot, ltree_node_t** node_out)
{
...
F_NONNULL
static void ns_add_glue_data(ltree_rrset_raw_t* glue_fake, const ltree_rrset_raw_t* target, const unsigned glue_name_offset, const unsigned rr_len, const bool in_deleg)
{
...
F_WUNUSED F_NONNULL
static bool ns_add_glue(ltree_rrset_raw_t* glue_fake, ltree_node_zroot_t* zroot, const uint8_t* node_dname, const uint8_t* name, unsigned name_len, unsigned glue_name_offset, const bool in_deleg)
{
    ltree_node_t* ns_target = NULL;
    ltree_dname_status_t target_status = ltree_search_name_zone(name, name_len, zroot, &ns_target);

    // Only attach glue from delegated spaces
    if (target_status != DNAME_DELEG)
        return false;

    ltree_rrset_raw_t* target_a = NULL;
    ltree_rrset_raw_t* target_aaaa = NULL;

    if (ns_target) {
        ltree_rrset_t* target_rrset = ns_target->c.rrsets;
        while (target_rrset) {
    ...
    gdnsd_assert(ns_target); // If this were false, we'd lack a+aaaa and fatal out above
...
bool comp_do_ns(ltree_rrset_raw_t* rrset, ltree_node_zroot_t* zroot, const ltree_node_t* node, const bool in_deleg)
{
    ...
    gdnsd_assert(dname_status(node->c.dname) == DNAME_VALID);
    ...
    for (unsigned i = 0; i < rrset->gen.count; i++) {
        ...
        if (stored_len == 2U) {
            ...
            gdnsd_assert(glue_name_offset & 0xC000u);
    ...
    if (rrset->num_rrsig) {
        ...
        if (cbuf_offset != input_offset) {
            ...
            gdnsd_assert(savings < rrset->rrsig_offset);
    ...
    if (glue_fake.gen.count) {
        ...
        if (glue_fake.num_comp_offsets) {
            gdnsd_assert(glue_fake.comp_offsets);
